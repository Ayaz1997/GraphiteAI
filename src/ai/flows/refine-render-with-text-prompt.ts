...
export async function refineRenderWithTextPrompt(
  input: RefineRenderWithTextPromptInput
): Promise<RefineRenderWithTextPromptOutput> {
...
const refineRenderWithTextPromptFlow = ai.defineFlow(
  ...
  async input => {
    const {media} = await ai.generate({
      ...
      prompt: [
        {
          text: `You are an AI that refines 3D renders based on user-provided text prompts.
  ...
  Refinement Prompt: ${input.textPrompt}`,
        },
    ...
    return {refinedRenderDataUri: media.url};
  }
);
